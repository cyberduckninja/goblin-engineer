...
struct data_t final {
    explicit data_t(std::uintptr_t id)
        : id_(id) {}
    std::uintptr_t id_;
...
public:
    ...
    void write(data_t d) {
        ...
        data_.response_.set(
            http::field::content_length, data_.response_.body().size());

        http::async_write(
            socket_, data_.response_, [self](beast::error_code ec, std::size_t) {
...
private:
    ...
    void read_request() {
        auto self = shared_from_this();
        http::async_read(
            socket_, buffer_, data_.request_, [self](beast::error_code ec, std::size_t bytes_transferred) {
    ...
    void process_request() {
        goblin_engineer::send(
            goblin_engineer::actor_address(address_), goblin_engineer::actor_address(address_), "router", std::move(data_));
    }
};
    ...
    explicit worker_t(goblin_engineer::intrusive_ptr<http_t> manager)
        ...
        add_handler("replay",
                    [&](goblin_engineer::context& /*ctx*/, data_t& data) -> void {
                        ...
                        goblin_engineer::send(
                            addresses("http"), address(), "write", std::move(data));
                    });
    }
